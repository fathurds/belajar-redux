...
export const updateProduct = createAsyncThunk("product/updateProduct", async ({ id, title, price }) => {
    const response = await axios.put(`http://localhost:5000/products/${id}`, { title, price });
    return response.data;
});
...
export const productSelectors = productEntity.getSelectors(state => state.product); // state.product sama dengan yang ada di store
